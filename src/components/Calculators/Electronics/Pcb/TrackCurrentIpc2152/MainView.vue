    ...
    <div class="diagram-container" style="position: relative; width: 600px; height: 450px;">
      ...
      <div class="variable-container" style="left: 500px; top: 300px;">
        ...
        <select v-model="calc.getVar('isPlanePresent').val" v-on:change="calc.getVar('isPlanePresent').onValChange()"
          ...
          <option v-for="option in calc.getVar('isPlanePresent').options" :key="option" v-bind:value="option">
            {{ option }}
          </option>
        </select>
      </div>
...
export default {
  ...
  data: function () {
      ...
      new CalcVarNumeric({
        ...
        eqn: () => {
          ...
          const universalChartTrendLineCoefA =
            ...
            Math.pow(tempRise, UNIVERSAL_CHART_TREND_LINE_COEF_AB)
          const universalChartTrendLineCoefB =
            UNIVERSAL_CHART_TREND_LINE_COEF_BA *
            Math.pow(tempRise, UNIVERSAL_CHART_TREND_LINE_COEF_BB)
        ...
        units: [
          ...
          new UnitMulti({
            name: 'mils²',
            multi: unitConversionConstants.M2_PER_MIL2
          }),
        ...
        eqn: () => {
          ...
          for (i = 0; i < trackThicknessTrendLineCoefA.length; i++) {
            trackThicknessModifierMulti +=
              trackThicknessTrendLineCoefA[i] * Math.pow(trackCurrentA, i)
          }
      ...
      new CalcVarNumeric({
        ...
        eqn: () => {
          ...
          const boardThicknessMils =
            boardThicknessM * (1 / unitConversionConstants.METERS_PER_MILS)
      ...
      new CalcVarNumeric({
        ...
        eqn: () => {
          ...
          const planeProximityMils =
            planeProximityM * (1 / unitConversionConstants.METERS_PER_MILS)
    ...
    calc.addVar(
      new CalcVarNumeric({
        ...
        rawVal: '0.20',
        units: [
          new UnitMulti({ name: 'W/(m*K)', multi: 1 }),
          new UnitMulti({
            name: 'BTU/(hour*ft*F)',
            multi:
              unitConversionConstants.THERMAL_CONDUCTIVITY_WATT_nMETER_nKELVIN_PER_BTU_nHOUR_nFT_nDEGF
          })
        ],
        ...
        validators: [
          ...
          new CustomValidator({
            func: () => {
              // Read dependency variables
              const thermalConductivity = calc
                .getVar('thermalConductivity')
                .getRawVal()
    ...
    calc.addVar(
      new CalcVarNumeric({
        ...
        eqn: () => {
          // Read dependency variables
          const unadjustedTrackCrossSectionalArea = calc
            .getVar('unadjustedTrackCrossSectionalArea')
            .getRawVal()
          const trackThicknessModifier = calc
            .getVar('trackThicknessModifier')
            .getRawVal()
          const boardThicknessModifier = calc
            .getVar('boardThicknessModifier')
            .getRawVal()
          const planeProximityModifier = calc
            .getVar('planeProximityModifier')
            .getRawVal()
          const thermalConductivityModifier = calc
            .getVar('thermalConductivityModifier')
            .getRawVal()
    ...
    calc.addVar(
      new CalcVarNumeric({
        ...
        eqn: () => {
          // Read dependency variables
          const trackThickness = calc.getVar('trackThickness').getRawVal()
          const adjustedTrackCrossSectionalArea = calc
            .getVar('adjustedTrackCrossSectionalArea')
            .getRawVal()

          const minimumTrackWidthM =
            adjustedTrackCrossSectionalArea / trackThickness
  ...
  methods: {
    drawCanvas: function (inputObj) {
      ...
      context.lineTo(
        topLeftX + pcbWidth / 2 + trackWidthBot / 2,
        topLeftY + copperThickness
      )
      context.lineTo(
        topLeftX + pcbWidth / 2 - trackWidthBot / 2,
        topLeftY + copperThickness
    ...
    openIpc2221ACalc: function () {
      this.$store.dispatch('openCalc', {
        componentName: 'track-current-ipc-2221a-calculator'
      })
    }
  },
  mounted () {
